...
<br>When `page=1`, `cursor` is an absolute physical record-slot offset and `count` is the maximum number of slots scanned inside the requested time window. The server may advance past the supplied cursor when complete files fall before `after`; continue with the returned cursor rather than adding the number of returned rows. Deleted records and records excluded by `uuid` or `sid` consume slots but are not returned, so a page may contain fewer than `count` rows or be empty. Repeat the same filters on every page.
...
| `X-OS-Page-Done` | `1` when the returned page reaches the end of the time window; otherwise `0`. |

Clients should retain the first response's window bounds and calculate progress as `(next - windowStart) / (windowEnd - windowStart)`. They must honor `X-OS-Page-Done`; a bounded time window can finish before `next` reaches `X-OS-Total-Slots`. Sensor logging continues during export; if a full ring rotates between page requests, its physical ordering can shift and produce a skipped or duplicated record. Paginated export is therefore best-effort rather than a transactional snapshot. Requests without `page=1` retain the original API behavior.
