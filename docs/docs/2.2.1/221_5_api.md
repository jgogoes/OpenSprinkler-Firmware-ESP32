## Firmware 2.2.1(5) API Reference [Jul 20, 2026]
...
* **External Sensor Framework**: A new set of endpoints manages external sensors (analog/ADS1115, aggregate, and weather sensors), their readings, and their logs: [`/jsn`](#24-get-sensors-jsn), [`/csn`](#25-add-or-change-sensor-csn), [`/dsn`](#26-delete-sensor-dsn), [`/jsl`](#27-get-sensor-log-jsl), [`/dsl`](#28-clear-sensor-log-dsl), and [`/jsd`](#29-get-sensor-descriptions-jsd). Sensors can scale program watering via adjustment curves — see [`/jpa`](#13a-get-program-adjustments-jpa). For the hardware and setup, see the [Sensor Expander](../sensor-expander/overview.md) documentation.
* **Extended Effective Runtimes**: Weather- and sensor-adjusted station runtimes may now exceed the 18-hour (65535-second) limit that applies to stored and manually-entered durations. The `rem` field in [`/jc`](#2-get-controller-variables-jc) `ps` and the `lrun` duration can therefore report values above `65535`.
...
|`lrbtc`| Cause code for the last reboot (see [List of Reboot Causes](#30-list-of-reboot-causes)).|
|`lrun` | Last run record: `[station_index, program_index, duration, end_time_epoch]`. The duration may exceed `65535` seconds after weather or sensor adjustment.|
...
|`wterr`| Error code from the last weather server response (see [List of Weather Error Codes](#31-list-of-weather-error-codes)).|
...
|`ps`   | Program status per station: each element represents a station consisting of four numbers `[pid, rem, start, gid]` where `pid`: program index (`0`: not running), `rem`: remaining seconds (possibly greater than `65535` after adjustment), `start`: start time, `gid`: sequential group ID. If a station’s `sbit=0` but `pid!=0`, it’s queued (waiting but not running yet). |
...
#### 13a. Get Program Adjustments `/jpa`

**Usage**: <code>http://os-ip/jpa?**pw**=xxx</code>

Returns the current weather (`wa`), sensor (`sa`), and combined (`ta`) adjustment factors for each program. `maxrt` is the maximum effective runtime in seconds for any individual station after applying these factors. Preview clients should calculate `min(base_duration * ta, maxrt)`.
...
Returns the list of configured external sensors and their latest readings. External sensors are a new feature in this firmware — see the [Sensor Expander](../sensor-expander/overview.md) documentation for hardware and setup details.

Each sensor is described by the following fields:

| Key | Meaning |
|:----|:--------|
| `uuid`   | Stable sensor ID (`1`–`65535`), preserved across add/delete/reorder. |
| `name`   | User-assigned sensor name. |
| `type`   | Sensor type: `0` Aggregate, `1` ADS1115, `2` Weather. |
| `unit`   | Unit index (see [`/jsd`](#29-get-sensor-descriptions-jsd)). |
| `flag`   | Bitmask — bit 0: enabled, bit 1: logging enabled. |
| `status` | Last read status (`0`: OK). |
| `interval`| Sampling interval, in minutes. |
| `min` / `max` | Output clamping range. |
| `value`  | Most recent reading. |
| `extra`  | Type-specific data (e.g. ADS1115 pin, Aggregate children). |

<br>**Example:** `{"sensors":[{"uuid":1,"name":"Soil 1","unit":0,"flag":3,"status":0,"interval":15,"min":0,"max":5,"value":2.7,"type":1,"extra":{...}}],"count":1}`

---

### 25. Add or Change Sensor `/csn`

**Usage**: <code>http://os-ip/csn?**pw**=xxx&**uuid**=xxx&**type**=xxx&...</code>

Adds a new sensor or modifies an existing one. Identify the target with either `uuid` (stable ID) or `sid` (positional index); if both are given, `uuid` takes precedence. Use `uuid=-1` (or `sid=-1`) to add a new sensor.
...
| `name`    | Sensor name. |
| `min` / `max` | Output clamping range. |
| `interval`| Sampling interval, in minutes (≥ 1). |
| `unit`    | Unit index. |
...
|:----------|:-----------|:--------|
| `children`| Aggregate | Semicolon-separated list of `uuid,scale,offset;` describing source sensors. |
| `action`  | Aggregate | Combine action: `0` Min, `1` Max, `2` Average, `3` Sum, `4` Median, `5` Range. |
| `pin`     | ADS1115   | Input pin (`1`–`16`). |
| `action`  | Weather   | Weather information index. |

---

### 26. Delete Sensor `/dsn`

**Usage**: <code>http://os-ip/dsn?**pw**=xxx&**uuid**=xxx</code>

Deletes a sensor. Identify the target with either `uuid` or `sid` (`uuid` takes precedence if both are given). Use `uuid=-1` (or `sid=-1`) to delete **all** sensors.
...
### 27. Get Sensor Log `/jsl`

**Usage**: <code>http://os-ip/jsl?**pw**=xxx&[**uuid**=xxx]&[**count**=xxx]&[**before**=xxx]&[**after**=xxx]&[**cursor**=xxx]&[**fmt**=xxx]&[**page**=1]</code>

Returns logged sensor readings. Records are stored oldest-first across a rotating set of log files; queries are served efficiently by skipping files that fall entirely outside the requested time window.

| Parameter | Meaning |
|:----------|:--------|
| `uuid`    | Filter to one sensor by stable ID (`1`–`65535`); `-1` (default) for all sensors. |
| `sid`     | Filter by positional index instead of `uuid` (`uuid` takes precedence). |
| `count`   | Max records to return. `max`/`all` returns up to the full capacity. |
| `before`  | Return only records with timestamp **before** this epoch value. |
| `after`   | Return only records with timestamp **after** this epoch value. |
| `cursor`  | Number of records to skip from the oldest, for pagination. |
| `fmt`     | Output format: `json` (default), `csv`, or `binary`. |
| `page`    | Set to `1` for reliable physical-slot pagination, including across deleted or sensor-filtered records. Cannot be combined with `before` or `after`. |
...
<br>**Example (json):** `[[1,1731000000,2.71],[1,1731000900,2.68]]`

<br>When `page=1`, `cursor` is a physical record-slot offset and `count` is the maximum number of slots scanned. Deleted records and records excluded by `uuid` or `sid` consume slots but are not returned, so a page may contain fewer than `count` rows or be empty. Time-window parameters (`before` and `after`) are not supported in page mode; omit `page` for those queries. Continue with the cursor supplied by the response rather than adding the number of returned rows:

| Response header | Meaning |
|:----------------|:--------|
| `X-OS-Next-Cursor` | Physical cursor for the next request. |
| `X-OS-Total-Slots` | Actual number of complete record slots present when the request began. |
| `X-OS-Page-Done` | `1` when the returned page reaches the end; otherwise `0`. |

Clients should retain `X-OS-Total-Slots` from the first page for progress reporting. Sensor logging continues during export; if a full ring rotates between page requests, its physical ordering can shift and produce a skipped or duplicated record. Paginated export is therefore best-effort rather than a transactional snapshot. Requests without `page=1` retain the original API behavior.

---

### 28. Clear Sensor Log `/dsl`
...
Clears logged readings for one sensor. Use `uuid=-1` to remove the complete sensor-log ring; the log header is recreated automatically when the next reading is logged. Without `page=1`, clearing one sensor scans the complete log synchronously for compatibility with existing clients.

With `page=1`, each request clears a bounded range and returns progress as `{"result":1,"next":819,"total":40950,"deleted":37,"done":0}`. `cursor` defaults to `0`; continue with the returned `next` value until `done` is `1`. `count` defaults to one log file's record capacity and is capped at that value. A page must be completed even when `deleted` is zero because matching records may occur later. Retrying the same cursor is safe. Clients should retain `total` from the first response, stop when `next` reaches that initial total, and limit the final request's `count` to `initialTotal - cursor` so records appended during the operation are not included.

Page mode requires a specific sensor UUID and cannot be combined with `uuid=-1`. To remove a sensor and its history, retain its UUID, delete the sensor with `/dsn`, and then complete the paginated `/dsl` operation so no new readings for that UUID are appended.

---

### 29. Get Sensor Descriptions `/jsd`

**Usage**: <code>http://os-ip/jsd?**pw**=xxx</code>

Returns descriptive metadata used by the UI to build the sensor configuration menus: the available sensor types, units, and per-type actions, along with their display names. The array index of each sensor type corresponds to its `type` value in [`/jsn`](#24-get-sensors-jsn) and [`/csn`](#25-add-or-change-sensor-csn); types disabled in this release are returned as stubs (with `"dis":1`) so positions stay stable.

---

### 30. List of Reboot Causes
...
### 31. List of Weather Error Codes
