...
* **Expanded Sensors**: New endpoints manage Expanded Sensors, their readings, metadata, and logs: [`/jsn`](#get-expanded-sensors-jsn), [`/csn`](#add-or-change-expanded-sensor-csn), [`/dsn`](#delete-expanded-sensor-dsn), [`/jsd`](#get-expanded-sensor-descriptions-jsd), [`/jsl`](#get-expanded-sensor-log-jsl), and [`/dsl`](#clear-expanded-sensor-log-dsl). Sensor readings can scale program water times through the sensor adjustment curves; see [`/jpa`](#get-program-adjustments-jpa). For analog sensor hardware and setup, see the [Sensor Expander](../sensor-expander/manual.md) documentation.
* **Sensor Terminology**:
    * **Built-in Sensors** refer to sensor ports on the main controller, identified by `SN1`–`SN4`. They support **digital** sensors such as rain, flow, and program switch.
...
| `sn?t`            | <span class="hl">Built-in Sensor type</span>, where `?` is `1`–`4`. Value: `0`: none, `1`: rain, `2`: flow, `3`: soil, `240`: program switch. Flow (`2`) is valid only for `sn1t`; <span class="hl">SN3/SN4 are available on OpenSprinkler v3.4+</span>.|
| `sn?o`            | <span class="hl">Sensor option</span>. `0`: normally closed; `1`: normally open (default). `?` is `1`-`4`, the same as `sn?t`.|
| `sn?on`/`sn?of`   | <span class="hl">Sensor activation / deactivation delay in minutes</span>. These delays apply only to rain and soil sensors. `?` is `1`-`4`, the same as `sn?t`.|
...
Each sensor has a stable ID (`uuid`) and a positional index (`sid`). The `uuid` is assigned when the sensor is created and remains associated with it when the sensor is deleted. The `sid` is the sensor's current index in the `sensors` array and may change when the array changes. Use `uuid` for durable references.
...
<br>**Example:** `{"sensors":[{"uuid":1,"name":"Soil 1","unit":0,"flag":3,"status":1,"interval":15,"min":0,"max":5,"value":2.7,"type":1,"extra":{"pin":1,"scale":1,"offset":0,"subtype":0}}],"count":1}`
...
* Continue with the returned `next` until `done` is `1`, **even when `deleted` is `0`** (matching records may appear in later slots).
